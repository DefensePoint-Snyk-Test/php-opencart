...
class Product extends \Opencart\System\Engine\Model {
	private function saveImageToFS(string $file): void {
	...
	public function addProduct(array $data): int {
		...
		if ($data['image']) {
			...
			if (isset($data['save_file'])) {
				$sql = "UPDATE `" . DB_PREFIX . "product` SET `image` = '" . (string)$data['image'] . "' WHERE `product_id` = '" . (int)$product_id . "'";
				$this->saveImageToDB($sql);

				$this->saveImageToFS($data['image']);
			}
		}
